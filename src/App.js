...
function categoriesForServiceSelection(selection) {
  const option = serviceCategoryOptions().find((item) => item.value === selection);
  ...
  const setRemarkStatus = (requestId, remarksStatusOrUpdates) => {
    ...
    const nextHistoryEntry = String(
      nextUpdates.historyEntry ||
        (hasRemarkChange
          ? `${nextUpdatedBy || actor} sent remarks back to ${nextRecipient || defaultRemarkRecipient(role)} on ${stamp}: ${nextRemarks}`
          : ''),
    ).trim();
...
function CustomerModal({ account, branches, box, plans: planOptions, save, close, existingCustomers }) {
  ...
  useEffect(() => {
    ...
    const nextGroup = groupedPlans[0];
  ...
  }, [groupedPlans, planOptions, selectedPlan]);
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const nextGroup = groupPlansByCategory(
                  planOptions
                    .map((item) => planCatalogLookup.get(normalizeCustomerName(item)) || { name: item })
...
function Linemans({ role, branch, setBranch, linemen, setLinemen, add, viewLineman, deleteLineman }) {
  const isBase = branch === 'base' || branch === 'All branches';
  const rows = linemen.filter((item) => isBase || item.branch === branch);
      ...
      <div className="section-title">
        ...
        <div className="lineman-header-actions">
          ...
          <label className="branch-filter">
            Branch
            <select
              value={isBase ? 'base' : branch}
              onChange={(event) => setBranch(event.target.value === 'base' ? 'base' : event.target.value)}
            >
              {['base', ...branches.filter((item) => item !== 'All branches')].map((item) => (
                <option key={item} value={item}>
                  {item}
                </option>
              ))}
